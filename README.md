...
A Retrieval-Augmented Generation (RAG) web application that answers questions about university lecture content using a local, open-source LLM. Built as a graduation project for the Business Technology BUA 304 (Predictive Analytics) course.
...
**Flow:** User asks a question in the Streamlit UI ? frontend sends it to the FastAPI `/query` endpoint ? backend embeds the question and retrieves the top-3 most similar chunks from Chroma ? backend builds a grounded prompt and calls Ollama ? the answer and its sources are returned to the frontend and displayed.
...
The knowledge base consists of **3 lecture PDFs (61 pages total)** from the university course **BUA 304: Predictive Analytics**, covering:
...
curl -X POST http://localhost:8000/query \
  ...
  -d "{\"question\": \"What is predictive analytics?\"}"
```
...
|---|---|---|---|
| 1 | What is predictive analytics? | ? | Exact match to slide definition |
| 2 | Who is the instructor of this course? | ? | Correct |
| 3 | What topics are covered in this course? | ? | Matches syllabus exactly |
| 4 | Why does Ridge Regression often perform better than OLS? | ? | Correct, cited |
| 5 | Why can Lasso set coefficients to exactly zero? | ?? | LLM formatting glitch (see below) |
| 6 | What is the key advantage of Elastic Net regression? | ? | Not answered in source slides (exercise-only question) — model correctly avoided fabricating |
| 7 | High training accuracy, poor test accuracy — bias or variance? | ? | Correct |
| 8 | What are the six stages of CRISP-DM? | ? | Retrieval miss — answer exists in corpus but wasn't retrieved |
| 9 | What is the difference between reducible and irreducible error? | ? | Correct, well-cited |
| 10 | What is the difference between supervised and unsupervised learning? | ? | Correct |
...
## Notes on Notebook Format

The RAG pipeline is implemented as `notebooks/rag_pipeline.py` (a plain Python script) rather than a `.ipynb` Jupyter notebook, due to a Visual Studio 2022 notebook-editor compatibility issue encountered during development. This format satisfies the "runs top-to-bottom, Kernel ? Restart & Run All" requirement even more strictly: `python notebooks/rag_pipeline.py` always executes every step, in order, in a single fresh process, with no possibility of out-of-order cell execution.

## Screenshots

*(Add screenshots here: Swagger UI showing a successful `/query` response, and the Streamlit chat interface answering a question.)*

## Track

**Core Track** — text-only RAG assistant. The Extended Track (Computer Vision/YOLO component) was considered but scoped out given the project's time constraints.